...
def train_model(
        ...
        learning_rate: float, reg_lambda: float, reg_gamma: float, reg_resi: float, reg_var: float,
        ...
        for phase in epoch_stats.keys():
            ...
            for idx, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    log_preds, loss_Resi = model(attr_mat_norm, idx)
                    ...
                    # Wq_reg = torch.sum(torch.abs(model.propagation.Wq.weight))
                    # Wk_reg = torch.sum(torch.abs(model.propagation.Wk.weight))
                    var_y = log_preds.size(0) - torch.trace(torch.exp(log_preds.T) @ torch.exp(log_preds))
                    loss = cross_entropy_mean + reg_lambda / 2 * l2_reg + reg_gamma / 2 * alpha_reg + reg_gamma / 2 * beta_reg + reg_resi * loss_Resi + reg_var * var_y
